...
{													//em que chegaram. 
	...
	for(int i : data){								//result guarda quantos cilindros foram percorridos, e é incrementado pelo módulo da subtração da posição atual(current) pelo próximo
		result += abs(current - i);					//elemento da fila (i). Após isto, a posição atual é atualizada.
		current = i;
	}
	...
	while(data.size() > 0){
		for(int i = 0; i < data.size(); i++){
			if(abs(current - data[i]) < shortestMove){	//Ao invés de apenas percorrer o vetor data uma vez, precisamos fazer isto múltiplas vezes. Em cada percorrimento, procuramos pelo
				shortestMove = abs(current - data[i]);	//seek que resulte no menor percorrimento. Quando terminamos de percorrer, efetuamos o movimento incrementando result,
				target = i;								//atualizamos current, removemos o pedido da lista e reiniciamos o processo novamente. Fazemos tudo isto até que a lista de pedidos esteja vazia.
			}
		}
		result += shortestMove;
		current = data[target];
	...
	while(goingDown){
		for(int i = 0; i < data.size(); i++){
			if( (data[i] < current) && ( abs(current - data[i]) < shortestMove) ){ //A implementação é similar a do SSF, exceto que para a primeira passagem, para que um pedido seja considerado,
				target = i;															//seu valor deve ser menor do que o da posição atual da cabeça.
		...
		if(shortestMove == (cilinders + 1)){				//Aqui usamos uma conveniência da implementação do SSF. Se após percorrer todo o vetor data, shortestMove ainda for o valor impossível,
			goingDown = false;								//isso indica que chegamos ao extremo dos pedidos nesta direção, então devemos mover a cabeça até a ponta. Representamos isto incrementando result e setando current para 0.
			result += current;
			current = 0;
			break;
		}
